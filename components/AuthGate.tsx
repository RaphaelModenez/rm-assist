...
export default function AuthGate({children}:{children:React.ReactNode}){
  ...
  return <div className="auth-screen">
    <form className="auth-card" onSubmit={entrar}>
      ...
      <p className="muted">Entre com seu e-mail e senha para acessar seus clientes e ordens de serviço.</p>
      ...
      <div className="field"><label>Senha</label><input type="password" autoComplete="current-password" value={senha} onChange={e=>setSenha(e.target.value)} placeholder="Sua senha"/></div>
      {erro&&<div className="error-box">{erro}</div>}
      <button className="primary-button auth-submit" disabled={enviando}>{enviando?"Entrando...":"Entrar"}</button>
    </form>
  </div>
}
