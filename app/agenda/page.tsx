      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:5,textAlign:"center"}}>
        ...
        {calendario.map((c,i)=>{
          ...
          return <button key={i} type="button" onClick={()=>{setDiaSelecionado(selecionado?null:c.iso);if(c.fora){const [a,m]=c.iso.split("-").map(Number);setMesVisivel(new Date(a,m-1,1))}}} aria-label={`${c.dia}, ${qtd} agendamento${qtd===1?"":"s"}`} style={{appearance:"none",border:selecionado?"2px solid #1677ff":"1px solid transparent",background:selecionado?"#eaf3ff":"transparent",borderRadius:12,minHeight:54,padding:"5px 2px",opacity:c.fora?.38:1,color:"inherit",position:"relative",cursor:"pointer"}}>
            ...
            <span style={{display:"flex",justifyContent:"center",gap:2,height:8,marginTop:2}}>{qtd>0&&<><i style={{display:"block",width:6,height:6,borderRadius:"50%",background:"#1677ff"}}/>{qtd>1&&<i style={{display:"block",width:6,height:6,borderRadius:"50%",background:"#1677ff",opacity:.55}}/>}</>}</span>
          </button>
        })}
      </div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,marginTop:8,minHeight:28}}><small className="muted"><span style={{display:"inline-block",width:7,height:7,borderRadius:"50%",background:"#1677ff",marginRight:6}}/>Dia com serviço agendado</small>{diaSelecionado&&<button type="button" onClick={()=>setDiaSelecionado(null)} style={{border:0,background:"transparent",color:"#1677ff",fontWeight:700,cursor:"pointer"}}>Ver todos</button>}</div>
    </section>
