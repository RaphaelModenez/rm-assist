...
export default function Historico(){
  ...
  const lista=useMemo(()=>{
    ...
    return items.filter((x:any)=>{
      const texto=[x.numero,x.clientes?.nome,x.clientes?.nome_fantasia,x.tipo_servico,x.situacao_final,x.equipamentos?.ambiente,x.equipamentos?.marca,x.equipamentos?.modelo].filter(Boolean).join(" ").toLowerCase();
  ...
  return <div className="page">
    ...
    <div className="service-list">{lista.map((o:any)=><article className="service-card" key={o.id}><div><span className="status-chip concluida">concluída</span><h3>OS #{String(o.numero).padStart(4,"0")} — {o.clientes?.nome_fantasia||o.clientes?.nome||"Cliente"}</h3><p>{o.tipo_servico||"Serviço"}{o.equipamentos?.ambiente?` • ${o.equipamentos.ambiente}`:""}</p>{o.situacao_final&&<small>{o.situacao_final}</small>}<small style={{display:"block",marginTop:4}}>{o.data_fim?new Date(o.data_fim).toLocaleString("pt-BR",{dateStyle:"short",timeStyle:"short"}):""}{o.valor_servico!==null&&o.valor_servico!==undefined?` • ${moeda(o.valor_servico)}`:""}</small></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Link href={`/os/${o.id}`} className="secondary-button">Abrir OS</Link><Link href={`/os/${o.id}/relatorio`} className="primary-button">Relatório</Link></div></article>)}</div>}
  </div>
}
