...
export default function Home(){
  const [s,setS]=useState({hoje:0,abertos:0,andamento:0,clientes:0,concluidos:0,atrasados:0});
  ...
  useEffect(()=>{(async()=>{
    ...
    setAgenda(
      ativos
        .filter((x:any)=>x.data_agendada && x.data_agendada>=hoje)
        .slice(0,4)
    );
    setLoading(false);
  })()},[]);
  ...
  return <div className="page">
    ...
    <section className="hero">
      <div><p className="eyebrow">PAINEL</p><h2>{saudacao}</h2><p>Seus atendimentos em um só lugar.</p></div>
    ...
    {loading?<p className="muted">Atualizando painel...</p>:<>
      {s.atrasados>0&&<Link href="/agenda" className="error-box" style={{display:"block",textDecoration:"none",marginBottom:14}}>
        <strong>{s.atrasados} atendimento{s.atrasados===1?"":"s"} atrasado{s.atrasados===1?"":"s"}</strong><br/>
        <span>Toque para revisar a agenda.</span>
      </Link>}
      ...
      <div className="stat-grid">
        ...
        <Link href="/clientes" className="stat-card" style={{textDecoration:"none"}}><strong>{s.clientes}</strong><span>Clientes ativos</span></Link>
      </div>
      ...
      {agenda.length===0?<p className="muted">Nenhum serviço agendado.</p>:<div className="agenda-list">
        {agenda.map(x=><Link href={destino(x)} className="agenda-card" key={x.id}>
          <div className="time-pill"><strong>{fmtData(x.data_agendada)}</strong><span>{x.hora_agendada?.slice(0,5)||"—"}</span></div>
          <div className="agenda-main"><strong>{nome(x)}</strong><span>{x.tipo_servico}</span></div>
          <span className="status-badge">{x.status==="em_atendimento"?"em atendimento":x.status}</span>
        </Link>)}
      </div>}

      <section className="quick-card">
        <div><p className="eyebrow">HISTÓRICO</p><h3>{s.concluidos} OS concluída{s.concluidos===1?"":"s"}</h3><p>Consulte atendimentos anteriores e relatórios.</p></div>
        <Link href="/historico" className="round-plus">›</Link>
      </section>
    </>}
  </div>
}
