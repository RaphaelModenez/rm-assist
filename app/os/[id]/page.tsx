...
export default function OSPage(){
  ...
  const [qtdEquipamentos,setQtdEquipamentos]=useState(0);
  ...
  useEffect(()=>{(async()=>{
    ...
    const [{data:c},{data:e},{data:ck},{data:mm},{data:ma},{data:fo},{data:rels}] = await Promise.all([
    ...
    setCliente(c); setEq(e); setQtdEquipamentos((rels||[]).length || (o.equipamento_id?1:0));
  ...
  async function saveExecucao(){
    ...
    try{
      const {data,error}=await s.from("ordens_servico")
        .update({servico_executado:servico,recomendacoes:os.recomendacoes||null,updated_at:new Date().toISOString()})
        .eq("id",id).select("id,servico_executado,recomendacoes").single();
      if(error)throw error;
      if(!data?.id)throw new Error("A OS não foi confirmada após o salvamento.");
      setOs((atual:any)=>({...atual,servico_executado:data.servico_executado,recomendacoes:data.recomendacoes}));
      setSalvo("Execução salva com sucesso"); setTimeout(()=>setSalvo(""),1800);
    }catch(e:any){setErro(e?.message||"Não foi possível salvar a execução.");}finally{setSalvando(false);}
  }
  ...
  async function removerFoto(f:any){const s=getSupabase(); if(!s)return;const {error:e1}=await s.storage.from("fotos-servico").remove([f.storage_path]);if(e1)return setErro(e1.message);const {error:e2}=await s.from("fotos_servico").delete().eq("id",f.id);if(e2)return setErro(e2.message);setFotos(fotos.filter(x=>x.id!==f.id));}
  function fotoUrl(f:any){return fotoUrls[f.storage_path]||"";}
  async function salvarLegendaFoto(f:any,legenda:string){const s=getSupabase(); if(!s)return setErro("Supabase não configurado.");setErro(""); setSalvo("");const texto=String(legenda||"").trim();const {data,error}=await s.from("fotos_servico").update({legenda:texto||null}).eq("id",f.id).select("id,legenda").single();if(error)return setErro(error.message);setFotos(atual=>atual.map((x:any)=>x.id===f.id?{...x,legenda:data?.legenda||""}:x));setSalvo("Legenda da foto salva");setTimeout(()=>setSalvo(""),1800);}
  async function saveChecklist(){const s=getSupabase(); if(!s)return;setSalvando(true); setErro("");await s.from("checklist_itens").delete().eq("ordem_servico_id",id);const rows=check.map(x=>({ordem_servico_id:id,categoria:x.categoria||null,item:x.item||x.descricao||null,status:x.status||null,observacao:x.observacao||null,concluido:x.status==="conforme"}));const {error}=await s.from("checklist_itens").insert(rows);setSalvando(false); if(error)setErro(error.message); else {setSalvo("Checklist salvo");setTimeout(()=>setSalvo(""),1800)}}
  async function saveMed(){const s=getSupabase(); if(!s)return;const payload={ordem_servico_id:id,retorno:numBR(med.retorno),insuflamento:numBR(med.insuflamento),tensao:numBR(med.tensao),corrente:numBR(med.corrente),pressao_succao:numBR(med.pressao_succao),pressao_descarga:numBR(med.pressao_descarga),superaquecimento:numBR(med.superaquecimento),subresfriamento:numBR(med.subresfriamento),delta_t:delta==="—"?null:numBR(delta),observacoes:med.observacoes||null,updated_at:new Date().toISOString()};const {data,error}=med.id?await s.from("medicoes").update(payload).eq("id",med.id).select("*").single():await s.from("medicoes").insert(payload).select("*").single();if(error)setErro(error.message); else {if(data)setMed(data);setSalvo("Medições salvas");setTimeout(()=>setSalvo(""),1800)}}
  async function addMat(){if(!material.descricao)return;const s=getSupabase(); if(!s)return;const q=numBR(material.quantidade)||1, vu=numBR(material.valor_unitario)||0;if(q<=0)return setErro("Informe uma quantidade válida.");const {data,error}=await s.from("materiais_servico").insert({ordem_servico_id:id,descricao:material.descricao,quantidade:q,valor_unitario:vu,valor_total:q*vu}).select("*").single();if(error)return setErro(error.message);setMats([data,...mats]); setMaterial({descricao:"",quantidade:"1",valor_unitario:""});}

  async function finalizar(){
    if(salvando||enviandoFoto)return;
    const s=getSupabase(); if(!s)return;
    if(qtdEquipamentos>1){
      const [{data:rels,error:eRels},{data:execs,error:eExecs}] = await Promise.all([s.from("ordem_servico_equipamentos").select("equipamento_id").eq("ordem_servico_id",id),s.from("os_equipamento_execucao").select("equipamento_id,situacao_final").eq("ordem_servico_id",id)]);
      if(eRels||eExecs)return setErro(eRels?.message||eExecs?.message||"Não foi possível validar os equipamentos.");
      const concluidos=new Set((execs||[]).filter((x:any)=>!!x.situacao_final).map((x:any)=>x.equipamento_id));
      const faltando=(rels||[]).map((x:any)=>x.equipamento_id).filter((equipamento_id:string)=>!concluidos.has(equipamento_id));
      if(faltando.length){const {data:eqFaltando}=await s.from("equipamentos").select("ambiente,tipo,marca").in("id",faltando);const nomes=(eqFaltando||[]).map((x:any)=>x.ambiente||x.tipo||x.marca||"Equipamento").join(", ");return setErro(`Ainda faltam ${faltando.length} equipamento${faltando.length===1?"":"s"} para concluir${nomes?`: ${nomes}`:""}. Volte em Equipamentos atendidos e preencha a Situação final.`);}
    }else if(!os.situacao_final){return setErro("Informe a situação final do equipamento antes de concluir a OS.");}
    const valor=numBR(os.valor_servico);if(os.valor_servico!=="" && os.valor_servico!==null && os.valor_servico!==undefined && valor===null)return setErro("Informe um valor de serviço válido. Você pode usar vírgula, por exemplo: 150,00.");
    setSalvando(true); setErro(""); setSalvo("");
    const fechamento={diagnostico:os.diagnostico||null,servico_executado:os.servico_executado||null,recomendacoes:os.recomendacoes||null,situacao_final:qtdEquipamentos>1?(os.situacao_final||"Todos os equipamentos atendidos"):(os.situacao_final||null),pendencias:os.pendencias||null,valor_servico:valor||0,forma_pagamento:os.forma_pagamento||null,responsavel_cliente:assinatura||null,status:"concluida",data_fim:new Date().toISOString(),updated_at:new Date().toISOString()};
    const {error}=await s.from("ordens_servico").update(fechamento).eq("id",id);if(error){setSalvando(false);return setErro(error.message)}
    if(os.chamado_id){const {error:e2}=await s.from("chamados").update({status:"concluido",updated_at:new Date().toISOString()}).eq("id",os.chamado_id);if(e2){setSalvando(false);return setErro("A OS foi concluída, mas o chamado não pôde ser atualizado: "+e2.message)}}
  ...
  return <div className="page">
    ...
    {erro&&<div className="error-box">{erro}</div>}{salvo&&<div style={{background:"#eaf8f0",color:"#18794e",border:"1px solid #c8ecd8",borderRadius:12,padding:10,marginBottom:14,fontSize:14,fontWeight:700}}>{salvo}</div>}
