...
function ServicosContent(){
 ...
 async function copiar(s:any,chamadoId:string,osId:string){
  const {data,error}=await s.from("chamado_equipamentos").select("equipamento_id").eq("chamado_id",chamadoId);if(error)throw error;
 ...
 async function iniciar(c:any){
  ...
  try{
   ...
   const copiados=await copiar(s,c.id,os.id);const {error:e2}=await s.from("chamados").update({status:"em_atendimento",updated_at:new Date().toISOString()}).eq("id",c.id);if(e2)throw e2;r.push(copiados.length>1?`/os/${os.id}/equipamentos`:`/os/${os.id}`);r.refresh();
